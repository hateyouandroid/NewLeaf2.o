...
class PinEntryEditText : AppCompatEditText {
    ...
    private var mLineSpacing = 0f //8dp by default, height of the text from our lines
    ...
    private fun init(context: Context, attrs: AttributeSet) {
        ...
        textEmptyPaint?.color = ContextCompat.getColor(context, R.color.purple_200)
        mLinesPaint = Paint(paint)
        mLinesPaint?.strokeWidth = mLineStroke
        if (!isInEditMode) {
            val outValue = TypedValue()
            context.theme.resolveAttribute(androidx.appcompat.R.attr.colorControlActivated, outValue, true)
            val colorActivated = outValue.data
            mColors[0] = colorActivated

            context.theme.resolveAttribute(androidx.appcompat.R.attr.colorPrimaryDark, outValue, true)
            val colorDark = outValue.data
            mColors[1] = colorDark

            context.theme.resolveAttribute(androidx.appcompat.R.attr.colorControlHighlight, outValue, true)
    ...
    private fun updateColorForLines(next: Boolean) {
        if (isFocused) {
          ...
          //  mLinesPaint?.color = ContextCompat.getColor(context, androidx.appcompat.R.color.material_blue_grey_800)
            if (next) {
          //      mLinesPaint?.color = ContextCompat.getColor(context, R.color.purple_500)
            }
        } else {
          //  mLinesPaint?.strokeWidth = mLineStroke
          //  mLinesPaint?.color = ContextCompat.getColor(context, R.color.purple_700)
        }
    }
